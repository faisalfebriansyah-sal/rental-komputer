...
const getRentals = async () => {
	try {
		...
		console.log("Response rental:", result);
		...
		console.error("Error rental:", err);
	}
};
...
const pelangganHariIni = computed(() => {
...
// Pelanggan yang sedang rental
const sedangRental = computed(() => {
	return rentals.value.filter((rental) => {
		return rental.status === "aktif";
	}).length;
});
					...
					<template v-if="!loading">
						<div class="rounded-2xl bg-white p-5 shadow-sm">
							<div class="flex items-center gap-4">
								<div
									class="flex h-11 w-11 items-center justify-center rounded-xl bg-[#91C8E4]/30 text-[#4682A9]">
									<Users :size="21" />
								</div>
								<div>
									<p class="text-sm text-slate-500">Total Pelanggan</p>
									<p class="mt-1 text-2xl font-bold text-slate-800">{{ totalPelanggan }}</p>
								</div>
							</div>
						</div>
						<div class="rounded-2xl bg-white p-5 shadow-sm">
							<div class="flex items-center gap-4">
								...
								<div>
									<p class="text-sm text-slate-500">Pelanggan Hari Ini</p>
									<p class="mt-1 text-2xl font-bold text-slate-800">{{ pelangganHariIni }}
									</p>
								</div>
							</div>
						</div>
						<div class="rounded-2xl bg-white p-5 shadow-sm">
							<div class="flex items-center gap-4">
								...
								<div>
									<p class="text-sm text-slate-500">Sedang Rental</p>
									<p class="mt-1 text-2xl font-bold text-slate-800">{{ sedangRental }}</p>
								</div>
							</div>
						</div>
					</template>
					<template v-else>
						<div class="rounded-2xl bg-white p-5 shadow-sm animate-pulse">
							<div class="flex items-center gap-4">
								<div
									class="flex h-11 w-11 items-center justify-center rounded-xl bg-[#91C8E4]/30 text-[#4682A9]">
									<Users :size="21" />
						...
						<div class="rounded-2xl bg-white p-5 shadow-sm animate-pulse">
							<div class="flex items-center gap-4">
								...
								<div class="flex-1">
									...
									<div class="h-6 w-18 bg-gray-300 rounded"></div>
								</div>
							</div>
						</div>
					</template>
						...
						<table class="w-full text-left">
							...
							<tbody>
								...
								<tr v-for="customer in filteredCustomers" :key="customer.id"
									...
									<td class="px-6 py-4">
										<span
											class="rounded-lg bg-[#91C8E4]/20 px-3 py-1.5 text-xs font-semibold text-[#4682A9]">
